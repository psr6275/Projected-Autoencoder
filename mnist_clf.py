...
class Mnist_CNN:
    ...
    def restore(self,ckpt_path = None):
        ...
        print("Success!")
    def targeted_attack(self,testX,testY,alpha = 0.3,max_iter=10,eps=8./255.):
        self.adv_imgs = tf.palceholder(tf.float32,shape=(None,28,28,1))
        labels_adv = tf.placeholder(tf.int32,shape=(10,))
        ...
        yte_adv = (np.argmax(Y_test,axis=1)+np.random.randint(low=1,hugh=9,size=(Y_test.shape[0],)))%10
